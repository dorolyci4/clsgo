...
func TestNewReusablePortListener(t *testing.T) {
	listenerOne, err := tcp.Listen("tcp4", "localhost:10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerOne.Close()

	listenerTwo, err := tcp.Listen("tcp", "127.0.0.1:10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerTwo.Close()

	listenerThree, err := tcp.Listen("tcp6", "[::]:10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerThree.Close()

	listenerFour, err := tcp.Listen("tcp6", ":10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerFour.Close()

	listenerFive, err := tcp.Listen("tcp4", ":10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerFive.Close()

	listenerSix, err := tcp.Listen("tcp", ":10081")
	if err != nil {
		t.Error(err)
	}
...
func TestNewReusablePortServers(t *testing.T) {
	...
	listenerTwo, err := tcp.Listen("tcp6", ":10081")
	if err != nil {
		t.Error(err)
	}
	defer listenerTwo.Close()
...
func BenchmarkNewReusablePortListener(b *testing.B) {
	for i := 0; i < b.N; i++ {
		listener, err := tcp.Listen("tcp", ":10081")

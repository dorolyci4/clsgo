...
func NewNetConn(address string, tcpreuse bool, timeout ...time.Duration) (net.Conn, error) {
	var (
		network  = `tcp`
	...
	var err error
	if tcpreuse {
		conn, err = DialTimeout(network, address, duration)
	} else {
...
// The optional parameter `timeout` specifies the timeout for dialing connection.
func NewNetConnTLS(address string, tcpreuse bool, tlsConfig *tls.Config, timeout ...time.Duration) (net.Conn, error) {
	var (
		network = `tcp`
		dialer  = &net.Dialer{
...
// dialing connection.
func NewNetConnKeyCrt(addr string, tcpreuse bool, crtFile, keyFile string, timeout ...time.Duration) (net.Conn, error) {
	tlsConfig, err := LoadKeyCrt(crtFile, keyFile)
	if err != nil {
		return nil, err
	}
	return NewNetConnTLS(addr, tcpreuse, tlsConfig, timeout...)
}
